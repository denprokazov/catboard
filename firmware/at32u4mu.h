...
#define LED_RED_CONFIG	(DDRF	|= (1<<4))
#define LED_RED_OFF		(PORTF	&= ~(1<<4))
#define LED_RED_ON		(PORTF	|= (1<<4))

#define LED_BLUE_CONFIG	(DDRB	|= (1<<2))
#define LED_BLUE_OFF	(PORTB	|= (1<<2))
#define LED_BLUE_ON		(PORTB	&= ~(1<<2))

#define FN_KEY_ID			7*5+4

// Init ports
void init_ports(void) {
	DDRB  = 0b00011000; DDRF  = 0b00000010; DDRD  = 0b11000000;
	PORTB = 0xFF;		PORTF = 0xFF; 		PORTD = 0xFF; PORTC = 0xFF;
}

// Pins keyboard matrix (have to properly initialize ports)
uint8_t *const	row_port[ROWS]	= { _PORTF,	_PORTB,	_PORTD,	_PORTB,	_PORTD};
const uint8_t	row_bit[ROWS]	= { (1<<1), (1<<3), (1<<7), (1<<4), (1<<6) };
uint8_t *const	col_pin[COLS]	= {	_PINB,	_PINB,	_PIND,	_PINC,	_PINB,	_PIND,		_PINB, _PINF,   _PINF,   _PINB,	_PINF,	_PINF }; //F1,B3,D7,B4,D6
const uint8_t	col_bit[COLS]	= { (1<<7),	(1<<1),	(1<<0),	(1<<6),	(1<<0),	(1<<1),	    (1<<5), (1<<0), (1<<7),	 (1<<6),	(1<<5),	(1<<6)}; //F6,F5,B6,D7,F0,B5,D1,B0,C6,D0,B1,B7
//d1 b0 c6 d0 b1 b7																	f6 f5 b6 d7 f0 b5
/*
